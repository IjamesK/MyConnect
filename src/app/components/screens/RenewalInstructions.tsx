...
const momoSteps = [
  ...
  "Enter Router serial number or number registered",
  "Chose desired package",
  "Enter number with money to pay or continue with the one being used ",
      ...
      <div className="px-4 py-5 space-y-4">
          ...
          <div className="space-y-3">
            <div className="bg-[#FCE7F3] border border-[#FBCFE8] rounded-2xl p-4">
              ...
              <p className="text-[#BE0067] text-xs mb-3">
                Enter your router serial number to link and pay
                online
              </p>
              ...
              <input
                ...
                placeholder="e.g. ZTEGD1234567890 or ALCL123456789"
                style={{
                  fontFamily: "'JetBrains Mono', monospace",
                }}
              ...
              <p className="text-[#94A3B8] text-[10px] mt-1.5">
                Found on the label on the back of your router
                (starts with ZTEGD or ALCL)
              </p>
            </div>
            <div className="bg-[#F0FDF4] border border-[#BBF7D0] rounded-xl p-3 flex items-center gap-2">
              ...
              <p className="text-[#15803D] text-xs">
                Secure payment via Stanbic, Centenary, or card
              </p>
            </div>
            <button
            ...
            {method === "momo" && (
              <div className="mt-3 bg-[#FFFBEB] border border-[#FDE68A] rounded-xl p-3 flex items-start gap-2">
                ...
                <p className="text-[#92400E] text-xs">
                  ...
                  *261# → 5 (Payments) → 2 (Pay Bill) → Existing
                  Customer → ZTEGD1234567
                </p>
              </div>
            )}
          </div>
        ...
        <div className="bg-white border border-[#E2E8F0] rounded-2xl p-4 flex items-center justify-between">
          <div>
            ...
            <p className="text-[#94A3B8] text-xs mt-0.5">
              Get SMS 3 days before expiry
            </p>
          </div>
          <button
            onClick={() => setAutoRenew(!autoRenew)}
            className={`w-12 h-6 rounded-full transition-colors relative ${autoRenew ? "bg-[#16A34A]" : "bg-[#CBD5E1]"}`}
          >
            <span
              className={`absolute top-1 w-4 h-4 bg-white rounded-full shadow transition-all ${autoRenew ? "left-7" : "left-1"}`}
            />
          </button>
        </div>
